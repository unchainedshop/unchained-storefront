 ...
 * Search assets by query, tags, type, or URL
 */
...
export default async function handler(
  ...
  try {
    const { q, tags, type, folderId, limit, url } = req.query;

    // URL-based lookup
    if (typeof url === "string" && url) {
      const asset = await findAssetByUrl(url);
      if (asset) {
        return res.status(200).json({ assets: [asset], total: 1 });
      }
    ...
    if (typeof q !== "string" || !q.trim()) {
      return res
        .status(400)
        .json({ error: "Search query (q) or url is required" });
    }
