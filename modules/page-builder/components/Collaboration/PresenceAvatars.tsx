  ...
  return (
    <div className={`flex items-center ${className}`}>
      <div className="flex -space-x-2">
        {visibleUsers.map((user, index) => (
          ...
          >
            ...
            <span
              ...
              style={{ backgroundColor: user.editingBlockId ? '#f59e0b' : '#22c55e' }}
            />
            ...
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-gray-900 text-white text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
              ...
              {user.editingBlockId && (
                <span className="text-amber-400 ml-1">
                  ({formatMessage({ id: 'pb_collaboration_editing', defaultMessage: 'editing' })})
                </span>
              )}
              {user.odId === localUser?.odId && (
                <span className="text-gray-400 ml-1">
                  ({formatMessage({ id: 'pb_collaboration_you', defaultMessage: 'you' })})
                </span>
              )}
            </div>
          </div>
        ...
        {hiddenCount > 0 && (
          <div
            className="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-xs font-medium text-gray-600 dark:text-gray-300 border-2 border-white dark:border-gray-800"
          >
            +{hiddenCount}
          </div>
        )}
      ...
      <span className="ml-3 text-sm text-gray-500 dark:text-gray-400">
        {formatMessage(
          { id: 'pb_collaboration_users_online', defaultMessage: '{count} online' },
